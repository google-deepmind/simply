...
class GenericInterface(Generic[RawT]):
  ...
  def __init__(
      ...
      # TODO: Remove min_prefill_size and move all usage to
      # SamplingParams.
    ...
    Args:
      ...
      pad_id: The pad id to use, if not given then it will use the `pad_id`
        field of the `vocab`.
      min_prefill_size: (Deprecated) The minimum prefill size to use (bos
        counted). Set in SamplingParams instead.
      max_input_len: The max input length to use (bos counted), longer inputs
        will be truncated to keep the last `max_input_len` tokens. If set to
        non-positive value then it will use the full input length. Note that,
        `max_input_len - 1` scores would be returned in `generate()`.
    ...
    if default_sampling_params is None:
      default_sampling_params = SamplingParams(
          min_prefill_size=min_prefill_size
      )
  ...
  def generate(
    ...
    Args:
      ...
      prefill_size: Prefill size to use for the generation, if set to a
        non-positive value, it will be inferred from the input length and
        self.min_prefill_size. At prefill stage, prefill_size of input tokens
        (bos counted) will be processed. Recommended to set to multiples of 128.
    ...
    for i in range(num_microbatches):
      ...
      # Encode the prompt and truncate to max_input_len (bos counted).
      inputs = []
      for txt in input_texts:
        if self.bos_id is not None:
          tokens = [self.bos_id, *self.vocab.encode(txt)]
        else:
          tokens = self.vocab.encode(txt)
        if self.max_input_len > 0:
          tokens = tokens[-self.max_input_len:]
        inputs.append(tokens)
      input_lens = [len(i) for i in inputs]  # bos counted
      max_input_len = max(input_lens)
      min_input_len = min(input_lens)
      assert min_input_len > 0

      # Pad inputs to max_input_len.
      for i, tokens in enumerate(inputs):
        inputs[i] = tokens + [self.pad_id] * (max_input_len - len(tokens))
      inputs = np.array(inputs)  # [batch_size, max_input_len]
      input_lens_np = np.array(input_lens)  # [batch_size]

      decoding_schedule = sampling_params.get_decoding_schedule(
          min_input_length=min_input_len, max_input_length=max_input_len
      )
      initial_length = 1 + max(
          decoding_schedule.get_next_length(max_input_len - 1),
          decoding_schedule.prefill_size,
      )
      inputs = pad_to_along_axis(
          inputs, initial_length, axis=1, constant_values=self.pad_id
      )

      # Repeat inputs to generate multiple samples. Ideally, this should be
      # after prefill. For simplicity, we do not want to handle the
      # replication of the decode state and logits.
      if sampling_params.num_samples > 1:
        inputs = einops.repeat(
            inputs, 'b t -> (b repeats) t', repeats=sampling_params.num_samples
        )
        input_lens_np = einops.repeat(
            input_lens_np,
            'b -> (b repeats)',
            repeats=sampling_params.num_samples,
        )
      # [batch_size]
      input_lens = input_lens_np.tolist()
      # [batch_size, 1]
      input_lens_np = np.reshape(input_lens_np, [-1, 1])

      position = decoding_schedule.begin_position
      logits, extra_output = self.prefill_fn(
          params,
          inputs[:, : decoding_schedule.prefill_size],
          position=position,
      ...
      if scoring_inputs:
        ...
        token_scores = compute_log_likelihood(
            logits,
            inputs[:, 1 : decoding_schedule.prefill_size + 1],
        ...
        token_scores = jnp.zeros(
            (inputs.shape[0], decoding_schedule.prefill_size), dtype=jnp.float32
        )
      ...
      sampling_state = SamplingState(
          ...
          token_logprobs=token_logprobs,
          token_scores=token_scores,
          input_lens=jnp.array(input_lens_np),
      )
      ...
      logging.info('max_input_len: %d', max_input_len)
      logging.info(
      ...
      assert isinstance(input_lens, list)
      assert isinstance(input_lens[0], int)
      for i in range(sampling_state.batch_size):
        ...
        for t, token_id in enumerate(raw_token_ids):
          if t >= min(
              input_lens[i] + sampling_params.max_decode_steps,
