...
import re
import time
from typing import Any, ClassVar, Tuple, TypeAlias, cast, final
...
@CheckpointFormatRegistry.register
@dataclasses.dataclass(frozen=True)
class GemmaLeapfrogFormat(Gemma2TransposeFormat):
  """Gemma checkpoint format by leapfrog."""
  ...
  def rules(self) -> Sequence[Tuple[str, ...]]:
    ...
    for rule in super().rules:
      if rule[0] not in (
          r'(.*)/block_(\d+)/attn/q_norm/(.*)',
          r'(.*)/block_(\d+)/attn/k_norm/(.*)',
      ):
        new_rules.append(rule)
    new_rules.extend([
        (
            r'(.*)/block_(\d+)/attn/q_norm/(.*)',  # s
            r'{prefix}/layer_\2/attn/query_norm/\3',  # s
        ),
        (
            r'(.*)/block_(\d+)/attn/k_norm/(.*)',  # s
            r'{prefix}/layer_\2/attn/key_norm/\3',  # s
        ),
    ])
    return new_rules
...
def load_checkpoint_from_manager(
  ...
  # TODO: Remove this once we fully deprecate the legacy format.
  checkpoint_manager._default_item.set(False)  # pylint: disable=protected-access
  if ckpt_step == -1:
    ckpt_step = checkpoint_manager.latest_step()
  t1 = time.time()
  logging.info(
      'Loading checkpoint from %s/%d', checkpoint_manager.directory, ckpt_step
  )
  item_metadata = checkpoint_manager.item_metadata(ckpt_step)
  formater = LegacyFormat()
  if ocp_constants.METADATA_ITEM_NAME in item_metadata:
    ...
    formater = metadata[CHECKPOINT_FORMAT_KEY]

  restore_args = ocp.checkpoint_utils.construct_restore_args(abstract_state)

  state_key = (
      'state' if 'state' in item_metadata else ocp_constants.DEFAULT_ITEM_NAME
  )
  restored = checkpoint_manager.restore(
      ckpt_step,
      args=ocp.args.Composite(**{
          state_key: ocp.args.PyTreeRestore(
              abstract_state,
              restore_args=restore_args,
              transforms=formater.transforms(
                  restore_args, original_metadata=item_metadata[state_key]
              ),
          ),
      }),
  )
  dt = time.time() - t1
  logging.info(
      'Checkpoint from %s/%d loaded with %f seconds spent.',
      checkpoint_manager.directory,
      ckpt_step,
      dt,
  )
  return restored[state_key]
...
def load_checkpoint_from_dir(
  ...
  with readonly_checkpoint_manager(ckpt_dir) as checkpoint_manager:
    ckpt = load_checkpoint_from_manager(
        checkpoint_manager, raw_abstract_state, ckpt_step
    )
  ckpt = common.transfer_metadata(abstract_state, ckpt)
  return ckpt


def load_data_state_from_dir(
    ckpt_dir: str,
    ckpt_step: int = -1,
) -> PyTree:
  ...
  with readonly_checkpoint_manager(ckpt_dir) as checkpoint_manager:
    item_metadata = checkpoint_manager.item_metadata(ckpt_step)
    if DATA_ITEM_NAME not in item_metadata:
      return {}
    restored = checkpoint_manager.restore(
        ckpt_step,
