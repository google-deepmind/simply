"""GMM v2 kernel implementation."""

# Source: https://github.com/vllm-project/tpu-inference/blob/main/tpu_inference/kernels/megablox/gmm_v2.py  # pylint: disable=line-too-long
# Forked into simply to avoid a cross-package visibility dependency on the
# tokamax/tpu_inference gmm_v2 kernel targets. Keep in sync with upstream.
...
def apply_act_fn(acc: jax.Array, fuse_act: str | None):
  ...
  Args:
      ...
      fuse_act: The name of the activation function to apply. Supported values
        are "silu", "gelu", and "swigluoai". If None, no activation is applied.
...
@dataclasses.dataclass(frozen=True)
class GmmConfigs:
  """GmmConfigs contains all the configs for the GMM kernel."""
    ...
    if cfgs.lhs_cfgs.quant_dtype is None:
      ...
      for start_n in range(0, rhs_tile_n, mxu_size):
        ...
        for b_id in range(cfgs.num_quant_blocks_per_tile_k):
          k_start = b_id * rhs_qbs  # pyrefly: ignore[unsupported-operation]
          k_end = k_start + rhs_qbs  # pyrefly: ignore[unsupported-operation]
...
def kernel_main(
  ...
  # Fill metadata buffer and return number of group & m interactions.
  ...
  if cfgs.zero_init:
    zero_out_end(out_ref, semaphore_ref, zero_size, dims=cfgs.dims)  # pyrefly: ignore[bad-argument-type]
